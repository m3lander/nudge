# Momo 📱 — a proactive AI agent for ADHD, living in iMessage

**You don't open Momo. Momo texts *you*.**

Every productivity app fails people with ADHD the same way: it waits patiently to be opened, and the whole problem is that it never gets opened. Momo inverts that. It's an accountability buddy with its own phone number that texts first — it breaks dreaded tasks into laughably small first steps, schedules its own check-ins, peeks at your screen for proof, calls you out (kindly) when you're doomscrolling, and celebrates real wins with full-screen confetti.

Built solo in one day at the 2026-07-18 hackathon. ~1,000 lines of TypeScript on Bun.

> **Demo video**: `momo-demo-3x.mp4` (submitted alongside; check-in intervals compressed with `TIME_SCALE` and playback at 3× — the agent behavior is unscripted).

## Why iMessage

People with ADHD don't need another app to forget. iMessage is already open, already checked compulsively, and — crucially — **notifications from a real phone number don't get ignored like app badges do**. The medium is the intervention: a text from Momo lands with the same weight as a text from a friend.
...
| **Self-scheduled check-ins** — the agent sets its own timers and texts you when they fire | Things silently dying in to-do lists. Momo owns the follow-up. |
...
| **Proof gates** — send a photo of the clean desk / packed bag; or grant Momo a 30-second-warning **screenshot peek** at your frontmost window | "I'll do it later" self-negotiation. The model actually looks at the image and judges honestly. |
...
| **No-reply watchdog** — ignore a check-in and Momo follows up on its own (twice, then backs off) | Ghosting your own commitments. Persistence is the product. |
...
| **Session kickoff** — `bun run demo` and Momo opens the conversation: "what are we working on right now?" | Blank-page mornings. You never have to initiate. |
...
| **Rejection-sensitive tone** — a missed check-in is data, never failure | RSD. Guilt-trippy reminders are actively harmful; Momo never sends one. |
...
bun run demo            # starts agent + distraction watcher, Momo texts you first
```
...
- **Task store as source of truth** — tasks.json + add/complete/list tools, so Momo resurfaces stale tasks days later (today's memory is per-session).
